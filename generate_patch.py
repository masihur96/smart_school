...
pattern = r'(Widget _buildClassPerformanceCard\(\s*BuildContext context,\s*MyClassAttendStudent stats,\s*\)\s*\{)(.*?)(^\s*Widget _buildStudentAvatarCard)'
replacement = r'\1\n    return _ClassPerformanceCardWithSubjectDropdown(stats: stats);\n  }\n\n\3'

new_content = re.sub(pattern, replacement, original, flags=re.DOTALL | re.MULTILINE)
