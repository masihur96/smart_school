...
class _TeacherPerformanceScreenState extends State<TeacherPerformanceScreen> {
  // Teacher search dropdown controller
  final TextEditingController _teacherSearchController =
      TextEditingController();
  final FocusNode _teacherSearchFocus = FocusNode();
  ...
  void initState() {
    ...
    _teacherSearchFocus.addListener(() {
      if (!_teacherSearchFocus.hasFocus) {
  ...
  void _showMonthPicker(TeacherPerformanceProvider provider) {
          ...
          builder: (context, provider, _) {
            return CustomScrollView(
              slivers: [
                ...
                SliverToBoxAdapter(
                  child: _buildTeacherDropdownSearch(provider),
                ),
                ...
                else if (provider.selectedTeacher != null)
  ...
  // ── Sliver App Bar ──────────────────────────────────────────────────────
  Widget _buildSliverAppBar(TeacherPerformanceProvider provider) {
    return SliverAppBar(
      expandedHeight: 150,
      floating: false,
      pinned: true,
  ...
  // ── Filter Bar ──────────────────────────────────────────────────────────
  Widget _buildFilterBar(TeacherPerformanceProvider provider) {
  ...
  // ── Teacher Dropdown Search ─────────────────────────────────────────────
  Widget _buildTeacherDropdownSearch(TeacherPerformanceProvider provider) {
    final allNames = provider.teacherNames;
                      ...
                      itemBuilder: (context, i) {
                        ...
                        return ListTile(
                          ...
                          onTap: () {
                            ...
                            provider.selectTeacherByName(name);
                          },
                        );
                      },
                    ),
          ...
          Container(
            ...
            child: Column(
              children: [
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (perf.designation.isNotEmpty)
                            _infoChip(perf.designation, Colors.purple),
                        ],
                      ),
                    ),
                    // Score badge
                ...
                // Two metric detail cards (Teachers don't have exams)
                Row(
                  children: [
                    Expanded(
  ...
  SliverList _buildRankedList(TeacherPerformanceProvider provider) {
  ...
  Widget _buildRankedCard(TeacherPerformance perf, {required int rank}) {
    final score = _score(perf);
    final color = _gradeColor(score);
    final label = _gradeLabel(score);
    ...
    return GestureDetector(
      onTap: () {
        _teacherSearchController.text = perf.name;
        setState(() {
          _teacherQuery = '';
          _showTeacherDropdown = false;
        });
  ...
  Widget _buildErrorState(TeacherPerformanceProvider provider) {
    return Padding(
